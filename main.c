...
void adf4351_wr_serial(unsigned long wr0, unsigned long wr1, unsigned long wr2,
                       unsigned long wr4)
{
...
void adf4351_wr_serialA(unsigned long wr0, unsigned long wr1, unsigned long wr2,
                        unsigned long wr4)
{
    ...
    while (1)
    {
        ...
        else if (flag6D > 0)
        {
            ...
            flag6D--;
                 ...
                 && (Buff[1] == 0x66)
                 && (len0 == 10)) // 0x8F 0x66    VFO
        {
            Freq = ((unsigned long) (Buff[2] - 0x30) * 100000000
        ...
        else if ((Buff[0] == 0x8F) && (Buff[1] == 0x78) && (len0 == 22)) // SINGLE
        {
            ...
            for (n = 0; n < Jjmps; n++)
            {
                ...
                // if((adcx1<130)&&(flag)) adcx=130; else
                // adcx=adcx1;//喜欢本底线平直就把下条语句这么处理；没意思
                adcx = adcx1;
                while (USART_GetFlagStatus(USART1, USART_FLAG_TXE) == RESET)
                {
                    /* wait */
