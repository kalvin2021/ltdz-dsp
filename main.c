...
/**
 * Network analyzer 4th order IIR bandpass filter coefficients.
 *
 * Coefficients generated by GNU Octave script:
 *
 *  [coeffs, shift] = cmsis_q31_bandpass_filter(f, 'cheby1')
 *
 * The coefficients are in the order required by the CMSIS
 * arm_biquad_cascade_df1_q31
 */

typedef struct na_iir_q31_filter_t
{
    unsigned fc_div10;
    q31_t    coeffs[10];
    unsigned shift;
...
#define NA_IIR_BIQUAD_ORDER     (4)
#define NA_IIR_BIQUAD_STAGES    (NA_IIR_BIQUAD_ORDER/2)

static const na_iir_q31_filter_t NA_IIR_Q31_BANDPASS_FILTER = {
    .fc_div10 = 80000 / 10,
    .coeffs = {
        11763, 23525, 11763, 1781780817, -1069066102,
        1073741824, -2147483647, 1073741824, 1787812329, -1069101510
    },
    .shift = 1
};

#define NA_IIR_BLOCK_SIZE   (1<<5)

STATIC_ASSERT_POWER_OF_2(NA_IIR_BLOCK_SIZE, "IIR filter block size must be 2**N");

static int16_t na_level_dB(void)
{
    const na_iir_q31_filter_t const* na_filter = &NA_IIR_Q31_BANDPASS_FILTER;

    arm_biquad_casd_df1_inst_q31 iir_biquad_filter;

    q31_t iir_biquad_state[NA_IIR_BIQUAD_STAGES * 4];

    arm_biquad_cascade_df1_init_q31(&iir_biquad_filter,
                                    NA_IIR_BIQUAD_STAGES,
                                    (q31_t*) na_filter->coeffs,
                                    iir_biquad_state,
                                    na_filter->shift);

    q31_t iir_block_buffer[NA_IIR_BLOCK_SIZE];

    uint64_t rms_squared_sum = 0;

    q15_t const* srcbuf = (q15_t const*)rx_adc_buffer;
    for (unsigned sample_count = 0; sample_count < CONFIG_RX_ADC_BUFFER_SIZE; sample_count += NA_IIR_BLOCK_SIZE)
    {
        /* Copy next sample block into the filter buffer */
        for (unsigned n = 0; n < NA_IIR_BLOCK_SIZE; n++)
        {
            iir_block_buffer[n] = (q31_t) ((*srcbuf++) << 16);
        }
         ...
         * Apply the filter to the filter buffer
         */
        arm_biquad_cascade_df1_q31(&iir_biquad_filter, iir_block_buffer, iir_block_buffer, NA_IIR_BLOCK_SIZE);
        ...
        for (unsigned n = 0; n < NA_IIR_BLOCK_SIZE; n++)
        {
            const uint32_t x = abs_q31(iir_block_buffer[n]);
            rms_squared_sum += ((uint64_t)x * x) >> 10;
        }
    }

    return log_dB(rms_squared_sum / CONFIG_RX_ADC_BUFFER_SIZE) - 1000;
}
